...
local function toggleFly()
    ...
    if flying then
        ...
        flyLoop = RunService.RenderStepped:Connect(function(delta)
            ...
            -- Vector horizontal de la cámara
            ...
            if moveVec.Magnitude > 0 then
                ...
                local targetOffset = moveDir * studs
            ...
            -- Rotación: siempre sigue horizontalmente la cámara
...
-- GUI
local function isGuiValid()
	return loopFlyButton and loopFlyButton.Parent and loopFlyButton.Parent:IsDescendantOf(game.CoreGui) or loopFlyButton.Parent:IsDescendantOf(localPlayer.PlayerGui)
end
...
-- Función para iniciar Fly
local function startFly()
	if not (humanoid and root) then resolveCharacter() end
...
-- Toggle Fly función
local function toggleFly()
	if not isGuiValid() then stopFly() return end
	flying = not flying
	if flying then
		if loopFlyButton then
...
-- Conectar botón MouseButton1Click
loopFlyButton.MouseButton1Click:Connect(toggleFly)

-- Tecla V alterna Fly como botón
UIS.InputBegan:Connect(function(input, gpe)
	if gpe then return end
...
-- Reconectar Fly al respawnear
localPlayer.CharacterAdded:Connect(function(char)
	resolveCharacter()
	stopFly() -- Reiniciar Fly y botón al morir/reseteo
end)

-- Reiniciar Fly al morir
humanoid.Died:Connect(function()
	stopFly()
end)

-- Detectar si GUI se elimina o cambia
if loopFlyButton then
	loopFlyButton.AncestryChanged:Connect(function()
...
-- Ajustes
local toggleOffsetY = 50 -- distancia desde top, ajusta como CoreGui
